...
class TimeBuf(object):
    ...
    def __init__(self, buf_length=2000, sample_rate=200):
        ...
        self.__data = Array("f", np.linspace(-t0, 0, self.buf_length, dtype=np.float32))
        self.__index_in = Value(ctypes.c_int16, 0)
        self.__index_out = 0
    ...
    def __getitem__(self, item):
        if isinstance(item, int):
            index = self.__index_in.value + item
    ...
    def asNumpy(self, latest=25000):
        dat = self.__data
        if self.__index_in.value > latest:
            return dat[self.__index_in.value - latest:self.__index_in.value]
        else:
            return np.concatenate((dat[self.buf_length + self.__index_in.value - latest:],
                                   dat[:self.__index_in.value]))
    ...
    def multiCoreProc(self):
        while self.__control.value:
            data = self.__pipe_queue.get()
            self.__data[self.__index_in.value] = data
            self.__index_in.value += 1
            if self.__index_in.value >= self.buf_length:
                self.__index_in.value = 0
    ...
    def updateOne(self, timestamp=None):
        ...
        self.__pipe_queue.put(timestamp)
